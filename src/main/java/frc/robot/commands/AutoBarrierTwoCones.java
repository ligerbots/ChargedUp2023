...
public class AutoBarrierTwoCones extends SequentialCommandGroup implements AutoCommandInterface {
    ...
    public AutoBarrierTwoCones(DriveTrain driveTrain, Arm arm, Vision vision, Claw claw, Position secondConePos) {
        ...
        addCommands(
            new ScoreArm(arm, Position.LEFT_TOP).withTimeout(5),
            new InstantCommand(claw::open),
            m_traj[0].alongWith(new ScoreArm(arm, Position.PICK_UP).withTimeout(5)),
            // new ScoreArm(arm, Position.PICK_UP).withTimeout(5),
